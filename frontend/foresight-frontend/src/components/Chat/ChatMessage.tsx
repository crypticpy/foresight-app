...
const isInternalPath = (s: string): boolean => INTERNAL_PATH_RE.test(s.trim());
...
function parseInline(
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1]) {
      // [text](url) link — internal routes use React Router, externals open in new tab
      ...
      // `code` — when content looks like an internal app route, render as a Link
      ...
      if (citation) {
        ...
        // Unmatched citation — still render as a styled "ghost" chip so the
