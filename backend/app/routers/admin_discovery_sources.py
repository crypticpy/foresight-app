"""Admin source-catalog CRUD (sub-router of ``admin_discovery``).

Owns ``/admin/sources`` + ``/admin/sources/categories``: the discovery source
registry (``discovery_sources_registry``) decorated with last-7d health
metrics from ``discovered_sources``. Mutating endpoints emit best-effort
audit-log entries via ``_safe_audit_log``.

Mounted by ``admin_discovery.py`` via ``router.include_router(...)`` so the
final URL prefix (``/api/v1``) is owned by the aggregator. Endpoint paths
in this file are written without that prefix.
"""
...
# No prefix — the parent ``admin_discovery`` router supplies ``/api/v1`` when
# it includes this sub-router.
