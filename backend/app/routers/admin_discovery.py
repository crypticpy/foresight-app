...
Owns the ``/admin/...`` discovery-side surface: coverage dashboards, goal
query refresh, balance dispatch, force-scan, run-detail lookup, and
schedule CRUD. Source-catalog CRUD (``/admin/sources*``) lives in the
sibling sub-router ``admin_discovery_sources.py`` and is mounted here via
...
from . import admin_discovery_sources

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/api/v1", tags=["admin-discovery"])

# Source-catalog CRUD (`/admin/sources*`) lives in a sibling sub-router so
# this file stays focused on coverage + ops + schedules. The sub-router has
# no prefix; the `/api/v1` prefix is applied at this aggregator boundary.
