...
def _fetch_card_snapshots(card_ids: List[str]) -> dict[str, dict]:
...
def _fetch_portfolio_or_404(portfolio_id: str, user_id: str) -> dict:
...
def _verify_workstream_visible(workstream_id: str, user_id: str) -> dict:
...
def _count_items(portfolio_id: str) -> int:
    res = (
...
def _list_items(portfolio_id: str) -> List[PortfolioItem]:
    res = (
        supabase.table("portfolio_items")
        .select("*")
        .eq("portfolio_id", portfolio_id)
    ...
    rows = res.data or []
    card_lookup = _fetch_card_snapshots([r["card_id"] for r in rows])
...
async def create_portfolio(
    ...
    if payload.workstream_id:
        _verify_workstream_visible(payload.workstream_id, user_id)
    ...
    if initial_card_ids:
        items = _add_items_to_portfolio(row["id"], initial_card_ids)
...
async def get_portfolio(
    ...
    row = _fetch_portfolio_or_404(portfolio_id, current_user["id"])
    items = _list_items(portfolio_id)
    return PortfolioWithItems(
        **_row_to_portfolio(row, item_count=len(items)).model_dump(),
        items=items,
    )
...
async def update_portfolio(
    ...
    user_id = current_user["id"]
    _fetch_portfolio_or_404(portfolio_id, user_id)
    ...
    return _row_to_portfolio(res.data[0], item_count=_count_items(portfolio_id))
...
def _add_items_to_portfolio(portfolio_id: str, card_ids: List[str]) -> List[PortfolioItem]:
    ...
    """
    if not card_ids:
        return _list_items(portfolio_id)
    ...
    if new_rows:
        supabase.table("portfolio_items").insert(new_rows).execute()
...
async def add_items(
    ...
    current = _count_items(portfolio_id)
    ...
    return _add_items_to_portfolio(portfolio_id, incoming)
...
async def reorder_items(
    ...
    for entry in payload.items:
        supabase.table("portfolio_items").update({"position": entry.position}).eq(
            "portfolio_id", portfolio_id
        ).eq("card_id", entry.card_id).execute()
...
async def export_portfolio(
    ...
    user_id = current_user["id"]
    row = _fetch_portfolio_or_404(portfolio_id, user_id)
